...
Node Nearest(Node curPos, vector<Node> unVisitList){
	...
	vector<double> ::iterator NearN = min_element(std::begin(distanceList),std::end(distanceList));
    auto NearPosition=distance(distanceList.begin(),NearN);
    Node NearestNode = unVisitList[NearPosition];
    
    return NearestNode;
}
 vector< astarPoint > move1(Node &curPos, Node toGo,vector<vector<int> > & grid ){
    //prepare point to point path planning : costmap
    vector<vector<int>> costmap = grid;
    vector< astarPoint > p ;
    int cols = int(grid[0].size());
    ...
    p=astar (curPos, curPos, toGo,  costmap );
    
    //directGoPath(costmap, grid, path, curPos, toGo);
    
    return p;
}
Node BetterPlowing(Node curPos, vector<Node> unVisitList,vector<vector<int> > & grid, vector<Node>& path){
	 vector<vector<astarPoint> > paths(unVisitList.size());
	 for(int i=0; i < unVisitList.size(); i++){
		 paths[i]=(move1(curPos, unVisitList[i],  grid )  );
	 }
	 int Min=0;
    ...
    Node NearestNode = unVisitList[Min];
	return NearestNode;
            ...
            if(TryTurn == 0)
			{
                ...
                if (checkFinish(grid) == false){
                    vector<Node> unVisitList = updateUnVisit(grid);
					 ...
					 Node toGo = BetterPlowing(curPos,unVisitList,grid,path) ;
        ...
        else
		{
            ...
            else
			{
                ...
                else
				{
					...
					else if(checkFinish(grid) == false)
					{ 
                        vector<Node> unVisitList = updateUnVisit(grid);
                       Node toGo = Nearest(curPos,unVisitList);
						 //Node toGo = BetterPlowing(curPos,unVisitList,grid,path);
